...
Paramètres radio, matériel, énergie et configuration retenus pour le maillage. Ces sections étaient auparavant dans le README ; elles en ont été sorties pour que la page d'accueil du dépôt reste lisible par des non-techniciens.
...
S'y ajoute une limite de capacité : le canal est unique et partagé, et avec les intervalles par défaut une soixantaine de nœuds suffit à le saturer avant même le premier message utile. Le projet retient donc un **plafond ferme de 50 nœuds**, soit environ 20 % de marge sous ce point de rupture. Ce n'est pas un objectif à atteindre puis à dépasser, c'est une règle d'exploitation. BlackOUT relie des points de ressource et des référents, pas 20 000 habitants : les 50 nœuds desservent des points d'eau, de distribution, de santé et de regroupement de quartier, et la population accède à l'information par l'affichage tenu à ces points. Le calcul de saturation figure dans dimensionnement-maillage.md, la répartition des 50 nœuds et le format des bulletins dans strategie-ressources.md.

La faiblesse d'authentification initialement signalée est traitée et n'appartient plus aux limites ouvertes. Les bulletins partent en messages directs chiffrés par clé publique et signés par la clé privée du nœud émetteur. La synthèse rediffusée porte une signature applicative calculée avec une clé distincte de la clé de canal, et les relais de toiture ne détiennent aucune clé privée de canal. Voir securite-authentification.md.
...
Documentation et blog officiels Meshtastic : réglages radio, configuration LoRa, configuration des appareils, algorithme de diffusion, choix des rôles, matériel supporté, tests de portée, chiffrement et configuration de sécurité.
